...
export default class Widget extends Component {
  ...
  constructor(props) {
    ...
    const invoiceId = router.query.id || this.props.id
    ...
    this.state = { 
      ...
      , wallet: props.wallet || {}
      , theme: (router.query.theme || props.theme) == 'light' ? 'light' : 'dark' 
  ...
  render() {
    ...
    const btcScheme = i => (`bitcoin:${i.address}?amount=${(i.price && i.price.btc)||0}`)
    const theme_reverse = theme=='dark'?'light':'dark'
    const css = `bg-${theme} text-${theme_reverse}`
    const bg = this.props.bg || false
    const { padded } = this.props
    const framed = this.props.framed || this.props.router.query.framed
    ...
    return (<div className={`${padded&&'py-5'} ${!framed&&padded&&'bg-mod bg-rainbow'}`}>
      ...
      { mounted && invoice.id && <div className={`widget ${theme} ${css} mx-auto noselect`}>
        ...
        <div className="p-3">
          ...
          { hasParent && <div onClick={()=>{ postMessage('close'); if(status == 'confirmed') postMessage({ status: 'success'}) }} className="close-widget d-flex justify-content-center align-items-center cursor">
          ...
          { status == 'pending' && <div className="text-center animated zoomInDown">
            ...
            <div className={`text-secondary mt-2 selectable ${invoice.address.length>34 ? 'fs-11':'fs-13'}`}>{invoice.address}</div>
        ...
        <div className={`powered px-3 pb-2 pt-4 d-flex flex-row-reverse justify-content-between`}>
          ...
          <div className="d-flex flex-column justify-content-center">
            <i onClick={()=>this.setState({ theme: theme_reverse })} className={`fa fa-square cursor text-${theme_reverse}`} />
          </div>
          {/*<div>#{invoice.id}</div>*/}
        </div>
...
const ProgressBar = ({ value, height = '10px', background = '#555' }) => (<div className="progress mb-1 mx-auto" style={{width: '200px',height: height, backgroundColor: background}}>
...
class NextBlock extends Counter {
  render() {
    ...
    const { expired, time_left, time_elapsed, percent } = timeDistance(start, end)
    const text = expired ? 'imminent' : time_left 
    return (<div className="text-center">
      <ProgressBar value={percent} />
      <small className="text-secondary">wait time: <span>{text}</span></small>
    </div>)
  }
}